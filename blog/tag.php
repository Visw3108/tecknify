...
<body>
  ...
  <section class="category-sec">
    ...
    if (!empty($currentTag)) {
      echo "<h2>Posts in Tag: " . htmlspecialchars($currentTag) . "</h2>";

      try {
        $stmt = $pdo->prepare("SELECT * FROM blogs WHERE FIND_IN_SET(:tag, tags) ORDER BY blog_post_date DESC");
        $stmt->bindParam(':tag', $currentTag);
        $stmt->execute();
        $posts = $stmt->fetchAll();
        ...
        if ($posts) {
          ...
          echo "<div class='center-message'><p>No posts available for the selected tag.</p></div>";
        }
      } catch (PDOException $e) {
        echo "<p>Error fetching posts: " . htmlspecialchars($e->getMessage()) . "</p>";
      }
    } else {
      echo "<p>Please select a tag to view posts.</p>";
    }
    ?>
  </section>
